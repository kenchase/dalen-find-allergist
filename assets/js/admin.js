...
var DalenFindAllergistAdmin = {
  ...
  validateSettings: function () {
    $('form').on('submit', function (e) {
      ...
      if (apiKey && !apiKey.match(/^AIza[0-9A-Za-z-_]{35}$/)) {
        isValid = false;
        errors.push(dalenAdmin.strings.invalidApiKey || 'Google Maps API key format appears to be invalid.');
      }
  ...
  initApiKeyTesting: function () {
    ...
    if ($('#google_maps_api_key').length) {
      var testButton = $('<button type="button" class="button button-secondary" id="test-api-key" style="margin-left: 10px;">' + dalenAdmin.strings.testApiKey + '</button>');
  ...
  initResetSettings: function () {
    $('.dalen-reset-settings').on('click', function () {
      if (confirm(dalenAdmin.strings.confirmReset)) {
        var button = $(this);
        button.prop('disabled', true).text(dalenAdmin.strings.resetting);
    ...
    $.ajax({
      ...
      .done(function (response) {
        if (response.status === 'OK') {
          button.removeClass('button-secondary').addClass('button-primary').text(dalenAdmin.strings.apiKeyValid);
          setTimeout(function () {
            button.removeClass('button-primary').addClass('button-secondary').text(dalenAdmin.strings.testApiKey).prop('disabled', false);
          }, 3000);
        } else {
          button.text(dalenAdmin.strings.apiKeyInvalid).css('color', '#dc3232');
          setTimeout(function () {
            button.text(dalenAdmin.strings.testApiKey).css('color', '').prop('disabled', false);
          }, 3000);
          console.error('API Key test failed:', response.status, response.error_message);
        }
      })
      .fail(function (xhr, status, error) {
        button.text(dalenAdmin.strings.testFailed).css('color', '#dc3232');
        setTimeout(function () {
          button.text(dalenAdmin.strings.testApiKey).css('color', '').prop('disabled', false);
        }, 3000);
